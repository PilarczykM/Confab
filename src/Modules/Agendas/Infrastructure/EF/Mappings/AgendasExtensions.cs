    ...
    internal static class AgendasExtensions
    {
        public static AgendaItemDto AsDto(this AgendaItem agendaItem) =>
            new()
            {
                ...
                Speakers = agendaItem.Speakers.Select(
                    s => new SpeakerDto { Id = s.Id, FullName = s.FullName }
                )
            };
        ...
        public static AgendaTrackDto AsDto(this AgendaTrack agendaTrack)
        {
            ...
            var regularSlots = agendaTrack.Slots
                .OfType<RegularAgendaSlot>()
                .Select(rs => (object)rs.AsDto());

            var placeholderSlots = agendaTrack.Slots
                .OfType<PlaceholderAgendaSlot>()
                .Select(
                    ps =>
                        (object)
                            new PlaceholderAgendaSlotDto
                            {
                                Id = ps.Id,
                                From = ps.From,
                                To = ps.To,
        ...
        public static RegularAgendaSlotDto AsDto(this RegularAgendaSlot slot) =>
            new()
            {
                ...
                AgendaItem = new AgendaItemDto
                {
                    ...
                    Speakers = slot.AgendaItem.Speakers.Select(
                        s => new SpeakerDto { Id = s.Id, FullName = s.FullName }
                    )
                }
            };
