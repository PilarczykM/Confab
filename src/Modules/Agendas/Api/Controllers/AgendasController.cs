...
namespace Confab.Modules.Agendas.Api.Controllers
{
    ...
    internal class AgendasController : BaseController
    {
        ...
        [HttpGet]
        [AllowAnonymous]
        public async Task<ActionResult<IEnumerable<AgendaTrackDto>>> GetAgendaAsync(
            Guid conferenceId
        ) =>
            OkOrNotFound(
                await _queryDispatcher.QueryAsync(new GetAgenda { ConferenceId = conferenceId })
            );

        [HttpGet("items")]
        [AllowAnonymous]
        public async Task<ActionResult<IEnumerable<AgendaItemDto>>> BrowseAgendaItemsAsync(
            Guid conferenceId
        ) =>
            OkOrNotFound(
                await _queryDispatcher.QueryAsync(
                    new BrowseAgendaItems { ConferenceId = conferenceId }
                )
            );
        ...
        [HttpPost("tracks")]
        public async Task<ActionResult> CreateAgendaTrackAsync(
            Guid conferenceId,
            CreateAgendaTrack command
        )
        {
        ...
        public async Task<ActionResult> AssignPlaceholderAgendaSlotAsync(
            AssignPlaceholderAgendaSlot command
        )
        {
            await _commandDispatcher.SendAsync(command);
            return NoContent();
        ...
        public async Task<ActionResult> AssignRegularAgendaSlotAsync(
            AssignRegularAgendaSlot command
        )
        {
            await _commandDispatcher.SendAsync(command);
            return NoContent();
