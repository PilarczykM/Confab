...
namespace Confab.Modules.Attendances.Api.Controllers
{
    ...
    internal class AttendancesController : BaseController
    {
        ...
        public async Task<ActionResult<IReadOnlyList<AttendanceDto>>> BrowseAttendancesAsync(
            Guid conferenceId
        )
        {
            var attendances = await _queryDispatcher.QueryAsync(
                new BrowseAttendances { ConferenceId = conferenceId, UserId = _context.Identity.Id }
            );
